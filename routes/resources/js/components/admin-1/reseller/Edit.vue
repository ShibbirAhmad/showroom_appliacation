...
export default {
  ...
  methods: {
    updateReseller() {
      this.form
        .post("/reseller/update/" + this.$route.params.id, {
          transformRequest: [
    ...
    getReseller() {
      axios.get("/reseller/" + this.$route.params.id).then((resp) => {
        if (resp.data.success == "OK") {
          ...
          this.form.image = resp.data.reseller.image;
        }
      });
    },
